...
export class WebApi {
  async getGroupEssenceMsg(GroupCode: string, page_start: string, CookieValue: string, Bkn: string) {
  ...
  async getGroupMembers(GroupCode: string, CookieValue: string, Bkn: string): Promise<WebApiGroupMember[]> {
  ...
  //   return await res.json();
  // }
  async setGroupNotice(GroupCode: string, Content: string = '', CookieValue: string, Bkn: string) {
  ...
  async getGrouptNotice(GroupCode: string, CookieValue: string, Bkn: string): Promise<undefined | WebApiGroupNoticeRet> {
    // const CookiesObject = await NTQQUserApi.getCookies('qun.qq.com');
    // const CookieValue = Object.entries(CookiesObject).map(([key, value]) => `${key}=${value}`).join('; ');
    // const Bkn = WebApi.genBkn(CookiesObject.skey);
  ...
  async getGroupHonorInfo(groupCode: string, getType: WebHonorType, CookieValue: string) {
